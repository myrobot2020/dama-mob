  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        ...
        if (req.method !== "POST" || (!isGemini && !isGroq && !isAuto)) {
          return next();
        }
        ...
        try {
          ...
          if ((process.env.GROQ_API_KEY ?? "").trim()) {
            try {
              const out = await callGroq(reflection, system, history);
              return sendJson(res, 200, out);
            } catch (e) { errorMessages.push(`Groq: ${e instanceof Error ? e.message : String(e)}`); }
          }
          ...
          if ((process.env.GEMINI_API_KEY ?? "").trim()) {
            try {
              const out = await callGemini(reflection, system, history);
              return sendJson(res, 200, out);
            } catch (e) { errorMessages.push(`Gemini: ${e instanceof Error ? e.message : String(e)}`); }
          }

          // Fallback to mock
          console.warn("[LLM Middleware] All providers failed or no keys configured. Returning mock response.", errorMessages);
