...
export class MockPlantClient implements PlantClient {
  ...
  async ingest(url: string) {
    console.log("Mock ingest for:", url);
    return { ok: true, output: "Mock discovery completed." };
  }
  ...
  private emit(
    verb: PlantEvent["verb"],
    job: InternalJob,
    payload?: Record<string, unknown>,
    extra?: Partial<PlantEvent>,
  ) {
    const e: PlantEvent = {
      id: nextId("evt"),
      ts: Date.now(),
        ...
        if (!this.wave3.pipeline.seal) {
          ...
          setTimeout(() => {
            ...
            this.emit(
              "seal.uploaded",
              job,
              { hash_id: sealArt.hash_id },
              { hash_id: sealArt.hash_id, model_version: sealArt.model_version },
            );
