...
export function useJob(id: string) {
  ...
  useEffect(() => {
    const client = getPlantClient();
    let mounted = true;
    const refresh = () => client.getJob(id).then((d) => mounted && setData(d));
    refresh();
    const off = client.subscribeEvents((e) => {
...
export function useArtifacts(filter: { hash_prefix?: string; model?: string; sutta_id?: string }) {
  const [arts, setArts] = useState<Artifact[]>([]);
  const key = JSON.stringify(filter);
  useEffect(() => {
    const client = getPlantClient();
    let mounted = true;
    const refresh = () => client.getArtifacts(filter).then((a) => mounted && setArts(a));
    refresh();
    const off = client.subscribeEvents((e) => {
