...
Examples:
    # Dry-run on Book 11
    python scripts/pipeline/10_keys.py --book 11 --dry-run

    # Force regeneration on Book 11
    python scripts/pipeline/10_keys.py --book 11 --force

    # Run with explicit glob
    python scripts/pipeline/10_keys.py --pattern "an11/suttas/*.json"

    # Multiple books (9 then 8 …), no in-run rolling few-shot, tallies per book + final
    python scripts/pipeline/10_keys.py --books "9,8,7,6,5,4,3" --rolling-examples 0 --model llama3:latest

    # Second pass: regenerate all, seed few-shot from existing on-disk chains per book, then rolling updates
    python scripts/pipeline/10_keys.py --books "9,8,7,6,5,4,3" --force --seed-examples 3 --rolling-examples 3 --model llama3:latest

    # Custom model / host
    OLLAMA_MODEL=llama3.2:3b python scripts/pipeline/10_keys.py --book 11
    OLLAMA_HOST=http://127.0.0.1:11434 python scripts/pipeline/10_keys.py --book 11
...
# Ensure REPO_ROOT is in sys.path so we can import scripts.pipeline.*
