...
Reads data/examples/sn/playlist_full.json (from scripts/pipeline/01_download.py).
...
Usage:
  python scripts/pipeline/03_identify.py
  python scripts/pipeline/03_identify.py --skip-subs     # only sync JSON + copy audio (subs already there)
  python scripts/pipeline/03_identify.py --subs-only     # only fetch subtitles
"""
...
_norm = _load_module(REPO / "scripts/pipeline" / "00_normalize.py", "norm_sn")
_cc = _load_module(REPO / "scripts/pipeline" / "11_clean.py", "cc_sn")
...
def sync_corpus(*, dry_run: bool) -> int:
    if not PLAYLIST_JSON.is_file():
        print(f"Missing {PLAYLIST_JSON}; run scripts/pipeline/01_download.py --manifest-only", file=sys.stderr)
        return 1
