...
from scripts.pipeline.utils import atomic_write_json
import importlib.util

# Load 19_generate_quiz.py dynamically or fix the import
def _load_quiz_gen():
    path = Path(__file__).resolve().parent / "19_generate_quiz.py"
    spec = importlib.util.spec_from_file_location("quiz_gen", str(path))
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod

quiz_gen = _load_quiz_gen()
ROOT = quiz_gen.ROOT
build_prompt = quiz_gen.build_prompt
call_ollama = quiz_gen.call_ollama
normalize_sutta_id = quiz_gen.normalize_sutta_id
validate_quiz = quiz_gen.validate_quiz
