...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--resume",
        action="store_true",
        help="Skip pages that already have extracted panels in the output directory.",
    )
    parser.add_argument(
        "--clean-output",
        action="store_true",
...
def extract_panels_from_pdf(
    ...
    with fitz.open(pdf_path) as document:
        ...
        for page_number, page in enumerate(document, start=1):
            ...
            if args.resume and volume_dir.exists():
                # Check if any panel files for this page already exist
                if any(volume_dir.glob(f"{volume}_p{page_number:04d}_panel*.png")):
                    continue

            if args.progress_every:
                elapsed = time.time() - start_time
                avg_per_page = elapsed / page_number if page_number > 0 else 0
                remaining_pages = page_limit - page_number
                ...
                print(f"\r[{volume}] 📄 {page_number}/{page_limit} | 🖼️ Panels: {len(panels)} | ⏳ ETA: {eta_str}   ", end="", flush=True)
