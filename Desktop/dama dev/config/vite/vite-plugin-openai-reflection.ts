...
// Simple manual parser for .env.local as a fallback
function loadEnvLocal() {
  try {
    ...
    console.error("[LLM Middleware] Failed to manual-load .env.local:", e);
  }
}
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on("end", () => {
      ...
      try {
        ...
        console.error("[LLM Middleware] JSON Parse Error:", e, "Raw:", raw.slice(0, 100));
        reject(new Error("Invalid JSON body."));
      }
    });
    req.on("error", (err) => {
      console.error("[LLM Middleware] Request Error:", err);
      reject(err);
    });
  });
}
...
function sendJson(res: import("node:http").ServerResponse, status: number, payload: any) {
  try {
    ...
    console.error("[LLM Middleware] sendJson Error:", e);
...
async function callGemini(
  ...
  const model = (process.env.GEMINI_REFLECTION_MODEL ?? "").trim() || "gemini-1.5-flash";
  ...
  contents.push({ role: "user", parts: [{ text: reflection.trim() }] });
...
async function callOpenAI(
  ...
  const body: OpenAIChatCompletionRequest = {
    ...
    messages: [
      ...
      ...history,
      { role: "user", content: reflection.trim() },
    ],
    max_tokens: 700,
    temperature: 0.7,
  };
...
async function callGroq(
  ...
  system: string,
  history: OpenAIChatMessage[] = [],
): Promise<{ answer: string; model: string; provider: "groq" }> {
  const apiKey = (process.env.GROQ_API_KEY ?? "").trim();
  if (!apiKey) {
    throw new Error("Groq is not configured. Set GROQ_API_KEY in .env.local.");
  }

  const model = (process.env.GROQ_REFLECTION_MODEL ?? "").trim() || "llama-3.3-70b-versatile";
  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        ...
        const isGroq = url === "/__groq/reflection";
        const isAuto = url === "/__llm/reflection";
        if (req.method !== "POST" || (!isOpenAi && !isGemini && !isGroq && !isAuto)) {
          return next();
        }
        ...
        try {
          ...
          if (isGroq) {
            const out = await callGroq(reflection, system, history);
            return sendJson(res, 200, out);
          }

          if (isOpenAi) {
            const out = await callOpenAI(reflection, system, history);
            return sendJson(res, 200, out);
          }

          if (isGemini) {
            const out = await callGemini(reflection, system, history);
            return sendJson(res, 200, out);
          }
          ...
          if ((process.env.GROQ_API_KEY ?? "").trim()) {
            try {
              ...
              errorMessages.push(`Groq: ${e instanceof Error ? e.message : String(e)}`);
            }
          }
          ...
          if ((process.env.OPENAI_API_KEY ?? "").trim()) {
            try {
              ...
              errorMessages.push(`OpenAI: ${e instanceof Error ? e.message : String(e)}`);
            }
          }
          ...
          if ((process.env.GEMINI_API_KEY ?? "").trim()) {
            try {
              ...
              errorMessages.push(`Gemini: ${e instanceof Error ? e.message : String(e)}`);
            }
          }

          // Fallback to mock
          console.warn(
            "[LLM Middleware] All providers failed or no keys configured. Returning mock response.",
            errorMessages,
          );
          return sendJson(res, 200, {
            answer: `[MOCK RESPONSE] You asked: "${reflection}". Since all configured API providers failed (or no keys were found), I'm providing this mock answer. ${errorMessages.length > 0 ? "Errors: " + errorMessages.join("; ") : ""}`,
            model: "mock-model",
            provider: "mock",
            used_llm: true,
          });
        } catch (e) {
          console.error("[LLM Middleware] Unhandled Error:", e);
          if (!res.writableEnded) {
            const msg = e instanceof Error ? e.message : String(e);
            sendJson(res, 500, { error: msg || "Unknown error." });
          }
        }
      });
    },
