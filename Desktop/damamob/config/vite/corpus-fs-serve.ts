...
let indexCache: { items: any[], searchRows: any[] } | null = null;
...
export function corpusFsMiddleware(dama5Root: string, audRoot: string) {
  return (
    ...
    if (raw === "/__dama_corpus__/index.json") {
      try {
        const now = Date.now();
        if (indexCache && (now - lastIndexLoad < CACHE_TTL_MS)) {
          res.setHeader("Content-Type", "application/json; charset=utf-8");
          res.end(JSON.stringify(indexCache));
          return;
        }

        // Optimization: if index.json already exists in the root, just serve it.
        const staticIndexPath = path.join(dama5Root, "index.json");
        if (fs.existsSync(staticIndexPath)) {
          const buf = fs.readFileSync(staticIndexPath);
          try {
            indexCache = JSON.parse(buf.toString());
            lastIndexLoad = now;
            res.setHeader("Content-Type", "application/json; charset=utf-8");
            res.end(buf);
            return;
          } catch (e) {
            console.warn("[CorpusFsMiddleware] Static index.json is invalid, rebuilding...");
          }
        }
        ...
        indexCache = { items, searchRows };
        lastIndexLoad = Date.now();

        res.setHeader("Content-Type", "application/json; charset=utf-8");
        res.end(JSON.stringify(indexCache));
      } catch (err) {
        console.error("[CorpusFsMiddleware] Error building index:", err);
        res.statusCode = 500;
        res.end();
      }
      return;
