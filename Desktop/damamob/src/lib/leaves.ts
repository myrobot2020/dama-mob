...
export function writeLeaves(next: LeavesStore) {
...
export function ensureLeaf(suttaId: string): Leaf {
  ...
  writeLeaves({ ...prev, [id]: leaf });
  return leaf;
}

export function mergeLeaves(incoming: LeavesStore): void {
  const prev = readLeaves();
  let changed = false;
  const next: LeavesStore = { ...prev };
  for (const [id, inc] of Object.entries(incoming)) {
    const cur = prev[id];
    if (!cur || inc.updatedAtMs > cur.updatedAtMs) {
      next[id] = inc;
      changed = true;
    }
  }
  if (changed) writeLeaves(next);
}
