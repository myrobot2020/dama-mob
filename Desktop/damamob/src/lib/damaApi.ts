...
export const DEFAULT_SUTTA_ID = "AN 1.19";
...
export function getCorpusAudSrc(filename: string): string {
  ...
  /** Direct access to Vite public/audio folder */
...
let corpusIndexPromise: Promise<{ items: ItemSummary[] }> | null = null;
let corpusIndexCachedAt: number | null = null;
...
export async function getItems(
  ...
  // Cache only the full index (no search query)
  if (!q && book === "all") {
    const now = Date.now();
    const isStale = !corpusIndexCachedAt || now - corpusIndexCachedAt > CORPUS_CACHE_TTL_MS;

    if (corpusIndexPromise && !isStale) {
      return corpusIndexPromise;
    }

    corpusIndexCachedAt = now;
    corpusIndexPromise = (async () => {
      try {
        const base = getDamaApiBase();
        const url = `${base}/api/items?book=all`;
  ...
  // If there is a query, bypass cache and fetch directly
  const base = getDamaApiBase();
  const usp = new URLSearchParams();
  if (q) usp.set("q", q);
  if (book) usp.set("book", book);
