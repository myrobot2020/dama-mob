...
export function AudioPlayer({
  ...
  const lastProgressRecordRef = useRef<number>(0);
  const PROGRESS_RECORD_INTERVAL_MS = 5_000;
  ...
  const tick = useCallback(() => {
    ...
    const now = Date.now();
    if (sid && clipLen > 0 && now - lastProgressRecordRef.current >= PROGRESS_RECORD_INTERVAL_MS) {
      lastProgressRecordRef.current = now;
    ...
    if (el.currentTime >= end - 0.05) {
      if (sid) recordAudioListenProgress(sid, 1);
      el.pause();
...
export function TrackedNativeAudio({
  ...
  useEffect(() => {
    ...
    const onTime = () => {
      ...
      const now = Date.now();
      if (now - lastProgressRecordRef.current >= PROGRESS_RECORD_INTERVAL_MS) {
        lastProgressRecordRef.current = now;
