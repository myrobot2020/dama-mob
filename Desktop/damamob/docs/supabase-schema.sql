...
-- Quiz results (per user, per sutta)
create table if not exists public.quiz_results (
  user_id uuid not null references auth.users (id) on delete cascade,
  sutta_id text not null,
  state text not null, -- grey, green, yellow, gold
  last_option_id text null,
  answered_at timestamptz null,
  yellow_at timestamptz null,
  fall_at timestamptz null,
  gold_at timestamptz null,
  updated_at timestamptz not null default now(),
  primary key (user_id, sutta_id)
);
